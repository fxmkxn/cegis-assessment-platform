...
// password. After the admin "goes live" with the instance they sign in here to:
//   1. set their 360 review panel (manager / reportees / up to 6 peers),
//   2. take the assessments assigned to them once each one's window is open, and
//   3. complete the WPCAS (Yes/No) reviews they were assigned as a rater.
// Auth is a lightweight token kept in localStorage (documented in the README).
// Plain language, single-line comments.

var PORTAL = { token: null, me: null, roster: [], picks: {} };
...
function showLogin() {
  ...
  render('<div class="auth-wrap" style="min-height:60vh">'
    ...
    + '<p>From here you set your 360 panel, take the assessments assigned to you, '
    + 'and complete the WPCAS reviews you have been asked to give.</p></div>'
...
function renderHome(d) {
  ...
  // step 1: the 360 panel task, shown first until it is submitted
  if (!d.panel_submitted) {
    blocks += '<div class="card pad" style="margin-bottom:14px;border-left:4px solid var(--ochre)">'
      ...
      + '<p class="muted small" style="margin-top:4px">Choose your manager, your reportees, and up to 6 peers. Your administrator confirms the final list of raters.</p></div>'
  ...
  // step 2: assessments assigned in this instance that are open right now
  var ass = d.assessments || [];
  var assHtml = ass.length ? ass.map(function (a) {
    var closes = a.deactivate_at ? "closes " + fmtDate(a.deactivate_at) : "no closing time";
    return '<div class="task"><div class="ic2">▦</div><div style="flex:1"><div style="font-weight:600">' + escHtml(a.title) + '</div>'
      + '<div class="muted small">' + (a.stage ? escHtml(a.stage.toUpperCase()) + " · " : "") + (a.time_limit_minutes ? a.time_limit_minutes + " min · " : "") + escHtml(closes) + '</div></div>'
      + '<button class="btn sm" onclick="launchKey(\'' + escAttr(a.access_key) + '\')">Start</button></div>';
  }).join("") : '<p class="muted small">No assessments are open for you right now. Check back when your administrator opens them.</p>';

  // step 3: WPCAS reviews this participant owes (only listed once they are open)
  ...
  }).join("") : '<p class="muted small">No WPCAS reviews are open for you yet.</p>';
  ...
  render('<div class="report-wrap">'
    ...
    + '<h3 style="margin:22px 0 10px">WPCAS reviews to complete</h3>' + taskHtml
    + '</div>');
}
...
function renderPanel(filter) {
  ...
  var rows = list.map(function (p) {
    ...
    return '<tr><td><b>' + escHtml(p.full_name) + '</b><div class="muted small">' + escHtml(p.email) + '</div></td>'
      + '<td class="muted small">' + escHtml([p.designation, p.department, p.location].filter(Boolean).join(" · ") || "—") + '</td>'
  ...
  var peers = Object.keys(PORTAL.picks).filter(function (k) { return PORTAL.picks[k] === "peer"; }).length;
  var mgrs = Object.keys(PORTAL.picks).filter(function (k) { return PORTAL.picks[k] === "manager"; }).length;
  var reps = Object.keys(PORTAL.picks).filter(function (k) { return PORTAL.picks[k] === "reportee"; }).length;
  ...
  render('<div class="report-wrap">'
    ...
    + '<div class="card pad" style="margin-bottom:12px"><input class="input" placeholder="Search people by name, team or city..." oninput="renderPanel(this.value)" value="' + escAttr(filter || "") + '"></div>'
    + '<div class="card"><table><thead><tr><th>Person</th><th>Role / city</th><th>Relationship</th></tr></thead><tbody>' + rows + '</tbody></table></div>'
...
function setPick(id, rel) { if (rel) PORTAL.picks[id] = rel; else delete PORTAL.picks[id]; }
function savePanel() {
  var byId = {}; PORTAL.roster.forEach(function (p) { byId[p.id] = p; });
  var picks = Object.keys(PORTAL.picks).map(function (id) {
    var p = byId[id];
    return { rater_id: id, rater_name: p.full_name, rater_email: p.email, relationship: PORTAL.picks[id] };
  });
