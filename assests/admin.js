...
function newForm() { withInstances(function () { shell(formView(null)); }); }
...
function renderDashboard() {
  ...
  if (!list.length) {
    rows = '<div class="card pad" style="text-align:center;padding:48px">'
      ...
      + '<p class="muted">Create your first instrument to get started.</p>'
      + '<button class="btn" style="margin-top:14px" onclick="go(\'new\')">＋ New assessment</button></div>';
  } else {
    var trs = list.map(function (a) {
      ...
      return '<tr>'
        ...
        + '<div class="muted small">v' + a.version + ' · created ' + new Date(a.created_at).toLocaleDateString() + '</div></td>'
        + '<td class="tnum">' + qc + '</td>'
        ...
        + '<button class="btn ghost sm" onclick="copyLink(\'' + a.id + '\')">Copy link</button>'
...
function formView(a) {
  ...
  var curInst = v("instance_id");
  var curStage = v("stage");
  ...
  var stages = ["", "baseline", "eoca", "endline", "wpca", "custom"];
  var stageOpts = stages.map(function (s) { return '<option value="' + s + '"' + (curStage === s ? " selected" : "") + '>' + (s === "" ? "— none —" : s) + '</option>'; }).join("");
  return '<div class="crumb">' + (isEdit ? "Edit assessment" : "New assessment") + '</div>'
    ...
    + '<div class="row2">'
    + '<div class="field"><label class="label">Assessment type</label><select class="input" id="f-type" onchange="document.getElementById(\'typehint\').textContent=this.value===\'wpca\'?\'WPCA: a 360 rating instrument. Upload rating-scale items; raters are assigned per subject.\':\'Objective: auto-scored MCQ / multi / true-false items.\'">'
    + '<option value="objective"' + (curType === "objective" ? " selected" : "") + '>Objective (MCQ / multi / true-false)</option>'
    + '<option value="wpca"' + (curType === "wpca" ? " selected" : "") + '>WPCA (360 rating)</option></select>'
    + '<div class="muted small" id="typehint" style="margin-top:4px">' + (curType === "wpca" ? "WPCA: a 360 rating instrument." : "Objective: auto-scored items.") + '</div></div>'
    + '<div class="field"><label class="label">Instance / cycle</label><select class="input" id="f-instance">' + instOpts + '</select></div>'
    + '</div>'
    + '<div class="row2">'
    + '<div class="field"><label class="label">Stage (optional)</label><select class="input" id="f-stage">' + stageOpts + '</select></div>'
    + '<div></div></div>'
...
function loadDetail(id) {
  ...
  Promise.all([
    ...
    shell('<div class="crumb"><a href="#" onclick="go(\'dashboard\');return false">Library</a> / ' + escHtml(a.title) + '</div>'
      ...
      + '<button class="btn ghost" onclick="copyLink(\'' + a.id + '\')">Copy link</button>'
      ...
      + kv("Type", a.assessment_type === "wpca" ? '<span class="tag" style="color:var(--blue-d);border-color:var(--blue)">WPCA (360)</span>' : "Objective")
      + kv("Status", a.is_active ? '<span class="pill live">Active</span>' : '<span class="pill closed">Inactive</span>')
      + kv("Access key", '<b style="font-family:ui-monospace,monospace;letter-spacing:1px">' + escHtml(a.access_key || "—") + '</b>' + (a.assessment_type === "wpca" ? ' <span class="muted small">(raters get their own keys after you finalise panels)</span>' : ' <button class="btn ghost sm" style="margin-left:8px" onclick="copyKey(\'' + escAttr(a.access_key || "") + '\')">Copy key link</button>'))
      + (a.stage ? kv("Stage", escHtml(a.stage)) : "")
      + kv("Version", "v" + a.version) + kv("Questions", qc) + kv(a.assessment_type === "wpca" ? "Ratings received" : "Respondents", rc)
...
function renderUpload() {
  ...
  var body;
  if (UPLOAD.step === 0) {
    var isW = a.assessment_type === "wpca";
    var cols = isW
      ? "no · q_competency · q_facet · q_stem · opt1-5 (scale labels) · marks"
      : "no · q_type · q_level · q_competency · q_facet · q_stem · opt1-5 · isopt1-5correct · marks";
    body = '<div class="card pad">'
      + '<div class="flex jb ac wrap" style="margin-bottom:14px"><h3>Upload ' + (isW ? "rating items" : "question") + ' file (.xlsx)</h3>'
      ...
      + '<div class="muted small">' + (isW ? "Each row is a rating scale (opt1..opt5 are the scale points). marks is the item's maximum." : "q_type must be mcqsca, mcqmca or tf. marks default to 0 if left blank.") + '</div></div>'
...
function handleFile(file) {
  ...
  reader.onload = function (ev) {
    ...
    var isW = ADMIN._assessment.assessment_type === "wpca";
...
function validationView() {
  ...
  var rows = UPLOAD.questions.map(function (q, i) {
    ...
    var nopts = [q.opt1, q.opt2, q.opt3, q.opt4, q.opt5].filter(function (o) { return o; }).length;
    return '<tr><td class="tnum">' + escHtml(q.no || (i + 1)) + '</td><td><span class="tag">' + escHtml(q.q_type || (ADMIN._assessment.assessment_type === "wpca" ? "rating" : "?")) + '</span></td>'
...
function previewView() {
  var cards = UPLOAD.questions.slice(0, 4).map(function (q) {
    return '<div class="card pad" style="margin-bottom:14px"><div class="flex jb"><span class="tag">' + escHtml(q.q_type) + '</span>'
...
function previewControls(q) {
  var isW = ADMIN._assessment && ADMIN._assessment.assessment_type === "wpca";
  ...
  var rating = q.q_type === "rating" || (isW && !q.q_type);
  return '<div style="pointer-events:none;opacity:.9">' + opts.map(function (o) {
    return '<div class="opt"><span class="rd' + (sq && !rating ? " sq" : "") + '"></span>' + escHtml(o) + '</div>';
  }).join("") + '</div>'
    + '<div class="muted small" style="margin-top:6px">' + (rating ? "Rating scale · " : "") + 'Marks: ' + escHtml(q.marks != null ? q.marks : 0) + '</div>';
}
function importView() {
  var a = ADMIN._assessment;
...
function doImport() {
  ...
  // for a WPCA assessment, blank question types are rating-scale items
  ...
  db.rpc("replace_questions", { p_assessment_id: id, p_questions: UPLOAD.questions }).then(function (r) {
    ...
    // store the original file (best effort; import already succeeded)
    if (UPLOAD._rawFile) {
      ...
      db.storage.from("assessment-files").upload(path, UPLOAD._rawFile, { upsert: true }).then(function (up) {
        ...
        hideLoader(); toast("Imported " + count + " questions (v" + newVersion + ")", "ok"); go("detail", id);
      });
    } else {
      hideLoader(); toast("Imported " + count + " questions", "ok"); go("detail", id);
    }
  });
}
function downloadTemplate() {
  ...
  var wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, isW ? "Rating items" : "Questions");
  XLSX.writeFile(wb, isW ? "cegis_wpca_template.xlsx" : "cegis_question_template.xlsx");
}
...
function renderResults() {
  ...
  if (!all.length) {
    shell(resultsHead(a) + '<div class="card pad" style="text-align:center;padding:48px">'
      ...
      + '<p class="muted">Share the link to start collecting responses.</p>'
      + '<button class="btn ghost" style="margin-top:14px" onclick="copyLink(\'' + a.id + '\')">Copy link</button></div>');
    return;
  }
  ...
  // competency averages from completed respondents' breakdowns are heavy to fetch;
  // here we chart the score distribution (always available) and offer per-person detail.
  ...
  db.rpc("admin_respondent_detail", { p_respondent_id: rid }).then(function (r) {
    ...
    secP.then(function (sec) {
      ...
      showModal({
        ...
        body: '<div class="muted small" style="margin-bottom:10px">' + escHtml(d.email) + ' · score <b>' + d.score_percent + '%</b> (' + d.score_raw + ' correct)' + (d.is_passed != null ? ' · ' + (d.is_passed ? "passed" : "failed") : "") + '</div>'
...
function exportResults() {
  ...
  var a = ADMIN._assessment, all = ADMIN._respondents;
  // pull responses + questions for completed respondents
  Promise.all([
  ...
  db.from("active_instances").select("*").order("created_at", { ascending: false }).then(function (r) {
    ...
    var rows = CACHE.instances.length ? CACHE.instances.map(function (i) {
      return '<tr onclick="go(\'instance\',\'' + i.id + '\')" style="cursor:pointer">'
        ...
        + '<td>' + (i.is_active ? '<span class="pill live">Active</span>' : '<span class="pill closed">Inactive</span>') + '</td>'
    ...
    }).join("") : '<tr><td colspan="4" class="muted" style="padding:20px;text-align:center">No instances yet. Create one to group assessments and run a 360 cycle.</td></tr>';
    shell('<div class="page-head"><h1>Instances</h1><button class="btn" onclick="instanceForm(null)">＋ New instance</button></div>'
      + '<div class="card"><table><thead><tr><th>Name</th><th>Status</th><th>Nomination opens</th><th>Rating opens</th></tr></thead><tbody>' + rows + '</tbody></table></div>');
  });
}
function instanceForm(inst) {
  ...
  shell('<div class="crumb"><a href="#" onclick="go(\'instances\');return false">Instances</a> / ' + (isEdit ? "Edit" : "New") + '</div>'
    ...
    + '<div class="row2">'
    + '<div class="field"><label class="label">Nomination opens</label><input class="input" id="i-nom-open" type="datetime-local" value="' + dt("nomination_opens_at") + '"></div>'
    + '<div class="field"><label class="label">Nomination closes</label><input class="input" id="i-nom-close" type="datetime-local" value="' + dt("nomination_closes_at") + '"></div></div>'
    + '<div class="row2">'
    + '<div class="field"><label class="label">Rating opens</label><input class="input" id="i-rate-open" type="datetime-local" value="' + dt("rating_opens_at") + '"></div>'
    + '<div class="field"><label class="label">Rating closes</label><input class="input" id="i-rate-close" type="datetime-local" value="' + dt("rating_closes_at") + '"></div></div>'
    + '<div class="switch"><input type="checkbox" id="i-active" ' + (inst.is_active ? "checked" : "") + '><label for="i-active">Active (visible as the working cycle)</label></div>'
...
function saveInstance(id) {
  ...
  var form = {
    ...
    is_active: document.getElementById("i-active").checked
  };
  showLoader("Saving...");
...
function renderInstance() {
  ...
  var tabs = [["overview", "Overview"], ["assessments", "Assessments"], ["participants", "Participants"], ["panels", "Panels & raters"], ["reports", "WPCA reports"]];
  ...
  shell('<div class="crumb"><a href="#" onclick="go(\'instances\');return false">Instances</a> / ' + escHtml(inst.name) + '</div>'
    ...
    + '<button class="btn ghost" id="instEditBtn">Edit</button></div>'
    + tabBar + body);
...
function instOverviewTab() {
  ...
  return '<div class="grid" style="grid-template-columns:1fr 1fr">'
    ...
    + kv("Status", i.is_active ? '<span class="pill live">Active</span>' : '<span class="pill closed">Inactive</span>')
    ...
    + kv("Rating opens", i.rating_opens_at ? fmtDate(i.rating_opens_at) : "—")
    + kv("Rating closes", i.rating_closes_at ? fmtDate(i.rating_closes_at) : "—") + '</div>'
    + '<div class="card pad"><h3 style="margin-bottom:12px">At a glance</h3>'
    + kv("Assessments", ADMIN._instAssessments.length)
    + kv("WPCA instruments", ADMIN._instAssessments.filter(function (a) { return a.assessment_type === "wpca"; }).length)
...
function instAssessmentsTab() {
  var rows = ADMIN._instAssessments.length ? ADMIN._instAssessments.map(function (a) {
    return '<tr><td><b>' + escHtml(a.title) + '</b></td>'
      + '<td>' + (a.assessment_type === "wpca" ? '<span class="tag" style="color:var(--blue-d);border-color:var(--blue)">WPCA</span>' : '<span class="tag">Objective</span>') + '</td>'
      + '<td class="muted small">' + escHtml(a.stage || "—") + '</td>'
      + '<td><span style="font-family:ui-monospace,monospace">' + escHtml(a.access_key || "—") + '</span></td>'
      + '<td>' + (a.is_active ? '<span class="pill live">Active</span>' : '<span class="pill closed">Inactive</span>') + '</td>'
  ...
  return '<div class="flex jb ac" style="margin-bottom:12px"><p class="muted small">Create an assessment and choose this instance, or set the instance on an existing one from its settings.</p>'
    + '<button class="btn sm" onclick="go(\'new\')">＋ New assessment</button></div>'
    + '<div class="card"><table><thead><tr><th>Title</th><th>Type</th><th>Stage</th><th>Key</th><th>Status</th><th></th></tr></thead><tbody>' + rows + '</tbody></table></div>';
}
...
function instParticipantsTab() {
  ...
  if (creds && creds.length) {
    ...
    credsBlock = '<div class="card pad" style="margin-bottom:14px;border-left:4px solid var(--green)">'
      ...
      + '<p class="muted small" style="margin:6px 0 10px">These passwords are shown once. Share them with each participant. You can reset a password later, but you cannot view it again.</p>'
  ...
  var plist = ADMIN._participants.length ? ADMIN._participants.map(function (p) {
    return '<tr><td><b>' + escHtml(p.full_name) + '</b><div class="muted small">' + escHtml([p.designation, p.department].filter(Boolean).join(" · ")) + '</div></td>'
  ...
  return credsBlock
    ...
    + '<div class="dz" id="pdz"><div style="font-size:30px">⤓</div><div style="font-weight:600;margin-top:6px">Drop the takers file here, or click to choose</div>'
    + '<div class="muted small">A password is auto-generated for each new person. Existing emails are skipped.</div></div>'
...
function instPanelsTab() {
  ...
  var wpca = ADMIN._instAssessments.filter(function (a) { return a.assessment_type === "wpca"; });
  var wpcaOpts = '<option value="">Choose a WPCA instrument…</option>' + wpca.map(function (a) {
  ...
  var panel = '<div class="card pad" style="margin-bottom:14px"><div class="row2">'
    ...
    + '<div class="field"><label class="label">WPCA instrument</label><select class="input" id="pn-assessment" onchange="ADMIN._panelAssessment=this.value">' + wpcaOpts + '</select></div></div>'
    + (wpca.length ? "" : '<p class="muted small" style="color:var(--red)">No WPCA instrument exists in this instance yet. Create one first (Assessments tab).</p>') + '</div>';
  return panel + '<div id="nomArea">' + (ADMIN._panelSubject ? '<p class="muted small">Loading nominations…</p>' : '<p class="muted small">Pick a subject to see their nominated raters.</p>') + '</div>';
}
function loadNominations() {
  ...
  Promise.all([
    ...
    }).join("") : '<tr><td colspan="4" class="muted" style="padding:16px;text-align:center">This subject has not nominated anyone yet.</td></tr>';
    ...
    var assignBlock = assigns.length ? '<div class="card pad" style="margin-top:14px"><h3 style="margin-bottom:10px">Finalised raters & their keys</h3>'
    ...
    area.innerHTML = '<div class="card pad"><div class="flex jb ac" style="margin-bottom:10px"><h3>Nominated raters</h3>'
      + '<button class="btn green sm" onclick="doFinalize()">Finalise selected as raters</button></div>'
      + '<p class="muted small" style="margin-bottom:10px">Tick the people who should rate this subject on the chosen WPCA instrument, then finalise. Each finalised rater gets a unique key.</p>'
...
function doFinalize() {
  ...
  if (!aid) { toast("Choose a WPCA instrument first", "err"); return; }
...
function instReportsTab() {
  ...
  return '<div class="card pad" style="margin-bottom:14px"><div class="field" style="max-width:420px"><label class="label">Subject</label>'
    ...
    + '<div id="reportArea">' + (ADMIN._reportSubject ? '<p class="muted small">Loading…</p>' : '<p class="muted small">Pick a subject to see their 360 report (self vs others, per competency).</p>') + '</div>';
}
function loadWpcaReport() {
  ...
  db.rpc("wpca_subject_report", { p_subject_id: ADMIN._reportSubject }).then(function (r) {
    ...
    if (!comps.length) { area.innerHTML = '<div class="card pad"><p class="muted">No completed 360 ratings for ' + escHtml(d.subject) + ' yet.</p></div>'; return; }
    ...
    area.innerHTML = '<div class="card pad" style="margin-bottom:14px"><div class="summary-band"><div><h2 style="margin:0">' + escHtml(d.subject) + '</h2>'
      + '<p class="muted small">360 review — self rating vs the average of all other raters</p></div>'
